...
console.log("engine-core: 3D render-space adapter");
{
  ...
  ok(
    ...
    "all walls retain one opaque mesh regardless of player position or fog state",
  );
  const playerOcclusionPolicy = resolvePlayerStructureOcclusionPolicy();
  ok(
    playerOcclusionPolicy.renderXrayPass &&
      playerOcclusionPolicy.keepStructuresOpaque &&
      playerOcclusionPolicy.xrayOpacity > 0 &&
      playerOcclusionPolicy.xrayOpacity < 1,
    "player readability uses a translucent x-ray pass instead of wall fading",
  );
