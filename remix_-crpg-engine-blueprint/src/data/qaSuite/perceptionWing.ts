// ── QA perception wing ───────────────────────────────────────────────────────
// A deliberately dark, generic acceptance room for the perception contract:
// authored light, sight, sound, smoke, memory/search, and LOS occlusion. The
// room avoids plot-specific language so it can remain an engine-level proof.

import {
  type CellOverrides,
  type QaWing,
  DOORWAY,
  WALL,
  entityPlacement,
  hostile,
  hubReturnExit,
  prop,
  roomCells,
  say,
  sign,
  stampCells,
  stampRect,
} from "./shared";

const perceptionCells = (() => {
  const o: CellOverrides = {};

  // The west L-baffle proves that sound can motivate a search without LOS.
  stampRect(o, -4, -8, -4, 1, WALL);
  stampCells(o, [[-4, -1]], DOORWAY);
  stampRect(o, -9, 1, -4, 1, WALL);
  stampCells(o, [[-7, 1]], DOORWAY);

  // The east divider hides the dark artifact and light-sensitive observer.
  stampRect(o, 4, -8, 4, 0, WALL);
  stampCells(o, [[4, -2]], DOORWAY);

  // A shutter wall leaves one direct sight lane down the room's center.
  stampRect(o, -3, -2, 3, -2, WALL);
  stampCells(o, [[0, -2]], DOORWAY);

  // Static smoke is authored with both hazard and semantic tag so perception
  // can recognize it without coupling this map to a bespoke object type.
  stampRect(o, 1, 0, 3, 2, {
    terrain: "smoke",
    hazard: "smoke",
    tag: "smoke_obscurance",
  });

  // Parallel approach lanes make movement sound legible without requiring a
  // debugger: hard stone is the normal/noisy control, while the soft lane is
  // quieter. Test each once normally and once with C-stealth active.
  stampRect(o, -2, 3, -2, 7, {
    terrain: "stone",
    tag: "qa_normal_noise_route",
  });
  stampRect(o, 2, 3, 2, 7, {
    terrain: "soft",
    tag: "qa_stealth_noise_route",
  });

  stampCells(o, [[0, 8]], { tag: "acceptance_dark_start" });
  stampCells(o, [[7, -7]], { tag: "acceptance_dark_artifact" });
  stampCells(o, [[-7, 4]], { tag: "acceptance_noise_source" });
  stampCells(o, [[0, 10]], DOORWAY);
  return roomCells(-10, 10, -10, 10, o);
})();

const perceptionMap = {
  id: "qa_perception_lab",
  display_name: "QA Perception Lab",
  width: 21,
  height: 21,
  ambient_light: 0,
  spawns: [
    {
      id: "spawn_return",
      cell: [0, 8] as [number, number],
      facing: [0, -1] as [number, number],
    },
  ],
  cells: perceptionCells,
  props: [],
  custom_object_placements: [
    sign("obj_terminal", [2, 8], "qa_dlg_perception_sign"),
    sign("obj_training_beacon", [-2, 8], "qa_dlg_hearing_route_sign"),
    {
      ...prop("obj_oil_lamp", [7, 2]),
      id: "qa_fixed_environment_lamp",
    },
    // A push, shove, or impact supplies a recognizable sound stimulus while
    // the west baffle prevents the hearing observer from seeing its source.
    {
      ...prop("obj_crate", [-7, 4]),
      id: "qa_noise_crate",
    },
  ],
  entity_placements: [
    entityPlacement("qa_sight_watcher", [0, -5], [0, 1]),
    entityPlacement("qa_sound_hunter", [-7, -5], [1, 0]),
    entityPlacement("qa_deaf_sentinel", [2, -5], [0, 1]),
    entityPlacement("qa_light_glass_watcher", [7, -5], [-1, 0]),
  ],
  item_placements: [
    {
      id: "qa_perception_portable_lamp",
      item_id: "qa_portable_lamp",
      cell: [0, 7] as [number, number],
      count: 1,
    },
    {
      id: "qa_perception_dark_artifact",
      item_id: "qa_dark_artifact",
      cell: [7, -7] as [number, number],
      count: 1,
    },
  ],
  container_placements: [],
  regions: [],
  triggers: [],
  exits: [hubReturnExit([0, 10])],
};

export const perceptionWing: QaWing = {
  maps: [perceptionMap],
  entities: [
    hostile(
      "qa_sight_watcher",
      "Sight-Dominant Watcher",
      { hp: 18, attack: 2, defense: 1, speed: 8, xp: 8 },
      {
        sensory_profile: {
          id: "qa_sight_dominant",
          memory_ticks: 150,
          search_ticks: 100,
          channels: [
            {
              id: "illuminated_sight",
              stimulus_kinds: ["visible_player"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {},
              range: 13,
              threshold: 0.12,
              sensitivity: 1.4,
              repeated_sound_gain: 0,
              positional_uncertainty: 0,
              barrier_response: "normal",
              requires_los: true,
              requires_view_cone: true,
              view_cone_degrees: 150,
              requires_illumination: true,
              tracks_live_target: true,
              source_tracking: "none",
            },
            {
              id: "secondary_hearing",
              stimulus_kinds: ["sound"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {
                footstep: 1,
                object_push: 1.1,
                impact: 1.1,
              },
              range: 7,
              threshold: 0.24,
              sensitivity: 0.8,
              repeated_sound_gain: 0.08,
              positional_uncertainty: 1.25,
              barrier_response: "normal",
              requires_los: false,
              requires_view_cone: false,
              view_cone_degrees: 120,
              requires_illumination: false,
              tracks_live_target: false,
              source_tracking: "none",
            },
          ],
        },
      },
    ),
    hostile(
      "qa_sound_hunter",
      "Hearing-Dominant Hunter",
      { hp: 18, attack: 2, defense: 1, speed: 9, xp: 8 },
      {
        sensory_profile: {
          id: "qa_hearing_dominant",
          memory_ticks: 180,
          search_ticks: 140,
          channels: [
            {
              id: "directional_hearing",
              stimulus_kinds: ["sound"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {
                footstep: 1.4,
                object_push: 1.35,
                impact: 1.5,
                stealth: 0.8,
              },
              range: 16,
              threshold: 0.08,
              sensitivity: 1.8,
              repeated_sound_gain: 0.22,
              positional_uncertainty: 0,
              barrier_response: "reduced",
              requires_los: false,
              requires_view_cone: false,
              view_cone_degrees: 120,
              requires_illumination: false,
              tracks_live_target: false,
              source_tracking: "none",
            },
            {
              id: "secondary_sight",
              stimulus_kinds: ["visible_player"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {},
              range: 7,
              threshold: 0.22,
              sensitivity: 0.8,
              repeated_sound_gain: 0,
              positional_uncertainty: 0,
              barrier_response: "normal",
              requires_los: true,
              requires_view_cone: true,
              view_cone_degrees: 110,
              requires_illumination: true,
              tracks_live_target: true,
              source_tracking: "none",
            },
          ],
        },
      },
    ),
    hostile(
      "qa_deaf_sentinel",
      "Deaf Visual Sentinel",
      { hp: 18, attack: 2, defense: 1, speed: 7, xp: 8 },
      {
        sensory_profile: {
          id: "qa_deaf",
          memory_ticks: 120,
          search_ticks: 90,
          // Deafness is represented by the absence of a sound channel. This
          // actor still sees normally, proving hearing is not a universal path.
          channels: [
            {
              id: "visual_only",
              stimulus_kinds: ["visible_player"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {},
              range: 9,
              threshold: 0.18,
              sensitivity: 1,
              repeated_sound_gain: 0,
              positional_uncertainty: 0,
              barrier_response: "normal",
              requires_los: true,
              requires_view_cone: true,
              view_cone_degrees: 125,
              requires_illumination: true,
              tracks_live_target: true,
              source_tracking: "none",
            },
          ],
        },
      },
    ),
    hostile(
      "qa_light_glass_watcher",
      "Light-Glass-Sensitive Watcher",
      { hp: 18, attack: 2, defense: 1, speed: 7, xp: 8 },
      {
        sensory_profile: {
          id: "qa_light_glass_sensitive",
          memory_ticks: 210,
          search_ticks: 160,
          channels: [
            {
              id: "glass_light_sense",
              stimulus_kinds: ["light", "fire"],
              stimulus_tags: ["glass", "lamp", "portable_light"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {
                glass: 1.35,
                portable_light: 1.2,
              },
              range: 18,
              threshold: 0.05,
              sensitivity: 2.2,
              repeated_sound_gain: 0,
              positional_uncertainty: 0,
              barrier_response: "normal",
              requires_los: true,
              requires_view_cone: false,
              view_cone_degrees: 120,
              requires_illumination: false,
              tracks_live_target: true,
              source_tracking: "lock_after_acquisition",
            },
            {
              id: "secondary_sight",
              stimulus_kinds: ["visible_player"],
              ignored_stimulus_tags: [],
              stimulus_tag_multipliers: {},
              range: 7,
              threshold: 0.22,
              sensitivity: 0.8,
              repeated_sound_gain: 0,
              positional_uncertainty: 0,
              barrier_response: "normal",
              requires_los: true,
              requires_view_cone: true,
              view_cone_degrees: 110,
              requires_illumination: true,
              tracks_live_target: true,
              source_tracking: "none",
            },
            {
              id: "secondary_hearing",
              stimulus_kinds: ["sound"],
              ignored_stimulus_tags: ["glass"],
              stimulus_tag_multipliers: {},
              range: 6,
              threshold: 0.28,
              sensitivity: 0.7,
              repeated_sound_gain: 0.05,
              positional_uncertainty: 2,
              barrier_response: "normal",
              requires_los: false,
              requires_view_cone: false,
              view_cone_degrees: 120,
              requires_illumination: false,
              tracks_live_target: false,
              source_tracking: "none",
            },
          ],
        },
      },
    ),
  ],
  dialogue: [
    say(
      "qa_dlg_perception_sign",
      "Perception Lab Instructions",
      "AMBIENT LIGHT IS ZERO. Dominant means specialty, not blindness: each observer retains weaker baseline senses except the explicitly Deaf Visual Sentinel, which has no sound channel. The center watcher has broad peripheral sight but still needs illumination and clear LOS. Pick up the Glass QA Lamp: the east watcher must first acquire it directly, then can trace that exact carried source briefly behind cover while it remains active; extinguishing or dropping it breaks the trace, and light evidence alone never starts combat. Push or strike the west crate; its physical noise should make the hearing hunter investigate around the L-baffle without learning your later hidden position. Cross the smoke patch to test obscurance, memory, and finite search. The Dark Artifact emits no light. Return through the south doorway.",
      [{ text: "Begin perception tests." }],
    ),
    say(
      "qa_dlg_hearing_route_sign",
      "Movement Noise and Stealth Route",
      "Use the west stone strip as the hard-surface control and the east soft strip as the quiet-surface control. Walk each route normally, then press C and repeat in Stealth: movement should become slower and quieter, but light and direct sight still reveal you. The center observer has ordinary hearing, the west hunter has strong hearing with lower uncertainty and reduced barrier loss, and the Deaf Visual Sentinel must never react to sound alone. Push the west crate to make a strong impact, then compare direct sound with sound routed around the west L-baffle. Leave Stealth before deliberate actions. Mechanical detection is independent of speaker volume.",
      [{ text: "Start the hearing checks." }],
    ),
  ],
  items: [
    {
      id: "qa_portable_lamp",
      display_name: "Glass QA Lamp",
      description:
        "A carried, placeable, throwable acceptance lamp. Its emitted stimulus exposes the carrier.",
      icon: "🏮",
      category: "key",
      spatial: { shape: [[0, 0]], weight_kg: 1.2, bulk: 1, stack_limit: 1 },
      simulation: {
        material_id: "glass",
        condition: "intact",
        integrity: 1,
        condition_tags: ["glass", "lamp", "noise_on_impact"],
        mass_kg: 1.2,
        bulk: 1,
        awkwardness: 0,
        push_difficulty: 1,
        carry_size: "hand",
        requires_cooperation: false,
      },
      light_source: {
        intensity: 0.9,
        radius: 14,
        color: "#ffd27a",
        active_by_default: true,
        extinguishable: true,
        mobility: "throwable",
        persistent: true,
        stimulus_tags: ["light", "lamp", "glass", "portable_light"],
        exposes_carrier: true,
      },
    },
    {
      id: "qa_dark_artifact",
      display_name: "Dark Artifact",
      description:
        "A deliberately non-emissive control item for proving that ordinary objects do not create light stimuli.",
      icon: "⬛",
      category: "key",
      spatial: { shape: [[0, 0]], weight_kg: 2, bulk: 1, stack_limit: 1 },
      simulation: {
        material_id: "stone",
        condition: "intact",
        integrity: 1,
        condition_tags: ["dark", "non_emissive", "artifact"],
        mass_kg: 2,
        bulk: 1,
        awkwardness: 0,
        push_difficulty: 1,
        carry_size: "hand",
        requires_cooperation: false,
      },
    },
  ],
};
